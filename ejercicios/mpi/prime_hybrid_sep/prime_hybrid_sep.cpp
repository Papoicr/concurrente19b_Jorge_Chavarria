...
int main(int argc, char* argv[])
{
	...
	#pragma omp parallel num_threads(thread_count) default(none) shared(my_rank, hostname, prime_count, std::cout)
	{
	...
	std::cout << "Process " << my_rank << " on " << hostname << " found " << prime_count << " primes in range [" << 
		my_start << ", " << my_finish << "[ in " << std::fixed << std::setprecision(9) << elapsed << 's' << 
